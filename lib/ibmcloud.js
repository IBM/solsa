...
const { SecretTransform } = require('./openwhisk')
  ...
  constructor ({ name, bindingFrom, topicName = name }) {
    ...
    this.Source = class extends Bundle {
      constructor ({ name, consumerGroup = name, sink }) {
        ...
        this.transformer = new SecretTransform({
          name: name,
          code: 'function main({ value }) { return { value: value.join() } }',
          input: bindingFrom.getSecret('kafka_brokers_sasl'),
          output: bootstrapServers.getValueFrom
        })
      }
    }
  }
