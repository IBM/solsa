...
class Service extends Bundle {
  ...
  getAllResources () {
    ...
    if (this.env) container.env = Object.keys(this.env).map(key => Object.assign({ name: key }, valueWrap(this.env[key])))
    if (this.ports) container.ports = Object.values(this.ports).map(containerPort => ({ name: 'http1', containerPort }))
