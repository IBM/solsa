...
class Ingress extends Bundle {
  ...
  getResources ({ config = {} } = {}) {
    ...
    let specs = (config.clusters || []).concat(config.contexts || []).filter(x => x.ingress)
    for (const spec of specs) {
      if (spec.ingress.iks) {
        ...
        resources.push({ obj: ingress, name: `ingress-${this.name}.yaml`, layer: spec.name })
        ...
        resources.push({ obj: route, name: `route-${this.name}.yaml`, layer: spec.name })
        ...
        resources.push({ JSONPatch: nodePortPatch, JSONPatchTarget: nodePortPatchTarget, layer: spec.name })
        ...
        resources.push({ obj: gw, name: `gw-${this.name}.yaml`, layer: spec.name })
        const vs = {
          apiVersion: 'networking.istio.io/v1alpha3',
          kind: 'VirtualService',
          metadata: {
        ...
        resources.push({ obj: vs, name: `vs-${this.name}.yaml`, layer: spec.name })
      }
    }
    return resources
  }
